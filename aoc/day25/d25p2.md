...
Paths to REACTOR_1:
1. START → A → C → REACTOR_1
2. START → A → B → C → REACTOR_1
3. START → B → C → REACTOR_1
4. START → B → A → C → REACTOR_1

Total paths: **4**

Note that START → A → B → A → C → REACTOR_1 is INVALID (visits A twice).
...
**This is the final challenge.** May your quantum calculations be swift! 🎄⚛️
...
- Maximum: **88+ sextillion** (88,629,381,196,525,010,959,290)

If you get a small number (< 1 billion), your algorithm isn't counting all paths correctly!

**Note:** The actual answer exceeds int64 capacity (~9.2 quintillion), requiring `math/big` for exact computation.
